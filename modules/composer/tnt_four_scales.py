...
from modules.composer.transformer import TransformerEncoderLayer
from modules.composer.misc import build_mlp
...
class TNT(Module):
    ...
    def forward(self, CLS, fine, middle, coarse, group, left_group_people_idx, right_group_people_idx):
        '''
        CLS: (1, B, d)
        fine: (F, B, d)
        middle: (M, B, d)
        ...
        for mod in self.layers:
            CLS_f, CLS_m, CLS_c, output_CLS, output_fine, output_middle, output_coarse, output_group = mod(
                output_CLS, output_fine, output_middle, output_coarse, output_group, left_group_people_idx, right_group_people_idx
            )
...
class TNT_Block(Module):
    def __init__(self, args, d_model):
        ...
        self.middle2group = Person_To_Group(args.N//2, d_model, args.source_version)
...
class Person_To_Group(Module):
    def __init__(self, num_person_per_group, d_model, source_version):
    ...
    def forward(self, person_feats_thisbatch_proj, left_group_people_idx, right_group_people_idx):
        ...
        if self.source_version:
            left_group_people_repre, right_group_people_repre = get_group_feats_source(person_feats_thisbatch_proj,
                                                                                       left_group_people_idx,
                                                                                       right_group_people_idx,
                                                                                       self.num_person_per_group)
        else:
            left_group_people_repre, right_group_people_repre = get_group_feats_fix(person_feats_thisbatch_proj,
                                                                                       left_group_people_idx,
                                                                                       right_group_people_idx)
        ...
        group_track_feats_thisbatch_proj = torch.stack(
            [left_group_feats_thisbatch_proj, right_group_feats_thisbatch_proj], dim=1)  # (B, 2, d)
